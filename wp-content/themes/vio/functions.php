...
if ( ! function_exists( 'vio_setup' ) ) :
	...
	function vio_setup() {
		 ...
		 * If you're building a theme based on vio, use a find and replace
		 * to change 'vio' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'vio', get_template_directory() . '/languages' );
		...
		*/
		...
		register_nav_menus(
			array(
				'top'             => esc_html__( 'Primary', 'vio' ),
				'side-panel-menu' => esc_html__( 'Side Panel Menu', 'vio' ),
				'flyout-menu'     => esc_html__( 'Flyout Menu', 'vio' ),
				'slideout-menu'   => esc_html__( 'Slideout Menu', 'vio' ),
			)
		);
		...
		add_theme_support( 'custom-background', $vio_args );
		 ...
		 * Redux custom css
		 */
		function vio_custom_redux_css() {
			/**
			 * [vio_custom_redux_css description]
			 */
			function vio_override_css_fonts_url() {
				$google_font_url = '';
				...
				*/
				if ( 'off' !== _x( 'on', 'Google font: on or off', 'vio' ) ) {
		...
		// This example assumes your opt_name is set to vio_theme_option, replace with your opt_name value.
		add_action( 'redux/page/vio_theme_option/enqueue', 'vio_custom_redux_css', 2 );
	}
endif;
add_action( 'after_setup_theme', 'vio_setup' );
 ...
 */
function vio_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'vio_content_width', 2000 );
}
add_action( 'after_setup_theme', 'vio_content_width', 0 );
 ...
 */
function vio_widgets_init() {

	// ADD MAIN SIDEBAR.
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'vio' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'vio' ),
	...
	if ( class_exists( 'woocommerce' ) ) {
		register_sidebar(
			array(
				'name'          => esc_html__( 'Product | Sidebar', 'vio' ),
	...
	register_sidebar(
		array(
			'name'          => esc_html__( 'Hamburger | Sidebar', 'vio' ),
			'id'            => 'radiantthemes-hamburger-sidebar',
			'description'   => esc_html__( 'Add widgets for "Hamburger" menu from here. To turn it on/off please navigate to "Theme Options > Header" and select "Hamburger" for respetive header styles.', 'vio' ),
...
add_action( 'widgets_init', 'vio_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function vio_scripts() {
...
add_action( 'wp_enqueue_scripts', 'vio_scripts' );
...
$radiantthemes_theme_options = get_option( 'vio_theme_option' );
ob_start();
...
/**
 * [vio_wrapper_start description]
 */
function vio_wrapper_start() {
	echo wp_kses_post( '<section id="main">' );
}
add_action( 'woocommerce_before_main_content', 'vio_wrapper_start', 10 );

/**
 * [vio_wrapper_end description]
 */
function vio_wrapper_end() {
	echo wp_kses_post( '</section>' );
}
add_action( 'woocommerce_after_main_content', 'vio_wrapper_end', 10 );

/**
 * [woocommerce_support description]
 */
function vio_woocommerce_support() {
	add_theme_support( 'woocommerce' );
}
add_action( 'after_setup_theme', 'vio_woocommerce_support' );

// Remove the product rating display on product loops.
remove_action( 'woocommerce_after_shop_loop_item_title', 'woocommerce_template_loop_rating', 5 );

// Ajax cart basket.
add_filter( 'woocommerce_add_to_cart_fragments', 'vio_iconic_cart_count_fragments', 10, 1 );

// Woocommerce product per page.
add_filter( 'loop_shop_per_page', 'vio_shop_per_page', 20 );
 ...
 */
function vio_shop_per_page( $cols ) {
	// $cols contains the current number of products per page based on the value stored on Options -> Reading
	// Return the number of products you wanna show per page.
	$cols = esc_html( vio_global_var( 'shop-products-per-page', '', false ) );
	return $cols;
}
/**
 * [vio_iconic_cart_count_fragments description]
 *
 * @param  [type] $fragments description.
 * @return [type]            [description]
 */
function vio_iconic_cart_count_fragments( $fragments ) {
	$fragments['span.cart-count'] = '<span class="cart-count">' . WC()->cart->get_cart_contents_count() . '</span>';
	return $fragments;
}

// Woocommerce wishlist button immediately after the add to cart button.
add_action( 'woocommerce_after_add_to_cart_button', 'vio_custom_action', 5 );

/**
 * Wistlist Button Beside Add To Cart Function.
 */

function vio_custom_action() {
...
function woocommerce_button_proceed_to_checkout() {
	?>
<a href="<?php echo esc_url( wc_get_checkout_url() ); ?>" class="checkout-button button alt wc-forward"><?php echo esc_html__( 'Checkout', 'vio' ); ?></a>
	<?php
}
 ...
 * Set Site Icon
 */
function vio_site_icon() {
	if ( ! function_exists( 'has_site_icon' ) || ! has_site_icon() ) {
		if ( vio_global_var( 'favicon', 'url', true ) ) :
			?>
			<link rel="icon" href="<?php echo esc_url( vio_global_var( 'favicon', 'url', true ) ); ?>" sizes="32x32" />
			<link rel="icon" href="<?php echo esc_url( vio_global_var( 'apple-icon', 'url', true ) ); ?>" sizes="192x192">
			<link rel="apple-touch-icon-precomposed" href="<?php echo esc_url( vio_global_var( 'apple-icon', 'url', true ) ); ?>" />
			<meta name="msapplication-TileImage" content="<?php echo esc_url( vio_global_var( 'apple-icon', 'url', true ) ); ?>" />
		<?php else : ?>
...
add_filter( 'wp_head', 'vio_site_icon' );
...
if ( ! function_exists( 'vio_pagination' ) ) {
 ...
 */
function vio_breadcrumbs() {

	$show_on_home = 0; // 1 - show breadcrumbs on the homepage, 0 - don't show
	if ( ! vio_global_var( 'breadcrumb_arrow_style', '', false ) ) {
		$delimiter = '<span class="gap"><i class="el el-chevron-right"></i></span>';
	} else {
		$delimiter = '<span class="gap"><i class="' . vio_global_var( 'breadcrumb_arrow_style', '', false ) . '"></i></span>';
	}

	$home         = esc_html__( 'Home', 'vio' ); // text for the 'Home' link.
	...
	if ( is_home() && is_front_page() ) {
		...
		if ( 1 === $show_on_home ) {
			echo '<div id="crumbs"><a href="' . esc_url( $home_link ) . '">' . esc_html__( 'Home', 'vio' ) . '</a></div>';
		}
		...
		echo '<div id="crumbs"><a href="' . esc_url( $home_link ) . '">' . esc_html__( 'Home', 'vio' ) . '</a> ' . wp_kses_post( $delimiter ) . ' ';
		if ( is_home() ) {
			...
			echo wp_kses_post( $before ) . esc_html__( 'Archive by category "', 'vio' ) . single_cat_title( '', false ) . '"' . wp_kses_post( $after );
		} elseif ( is_search() ) {
			echo wp_kses_post( $before ) . esc_html__( 'Search results for "', 'vio' ) . get_search_query() . '"' . wp_kses_post( $after );
		} elseif ( is_day() ) {
			...
			echo wp_kses_post( $before ) . esc_html( vio_global_var( 'event_banner_title', '', false ) ) . wp_kses_post( $after );
			...
			echo wp_kses_post( $before ) . esc_html__( 'Posts tagged "', 'vio' ) . single_tag_title( '', false ) . '"' . wp_kses_post( $after );
		} elseif ( is_author() ) {
			global $author;
			$userdata = get_userdata( $author );
			echo wp_kses_post( $before ) . esc_html__( 'Articles posted by ', 'vio' ) . esc_html( $userdata->display_name ) . wp_kses_post( $after );
		} elseif ( is_404() ) {
			echo wp_kses_post( $before ) . esc_html__( 'Error 404', 'vio' ) . wp_kses_post( $after );
		}
		...
		if ( get_query_var( 'paged' ) ) {
			...
			echo esc_html_e( 'Page', 'vio' ) . ' ' . get_query_var( 'paged' );
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
				echo ')';
			}
		}
...
if ( ! function_exists( 'vio_import_files' ) ) :

	/**
	 * [vio_import_files description]
	 */
	function vio_import_files() {

		return array(
			array(
				'import_file_name'         => esc_html__( 'Demo One', 'vio' ),
				...
				'preview_url'              => 'https://vio.radiantthemes.com/',
			),
		);

	}
	add_filter( 'pt-ocdi/import_files', 'vio_import_files' );
endif;

if ( ! function_exists( 'vio_after_import' ) ) :
	/**
	 * [vio_after_import description]
	 *
	 * @param  [type] $selected_import description.
	 */
	function vio_after_import( $selected_import ) {
	...
	add_action( 'pt-ocdi/after_import', 'vio_after_import' );
endif;
 ...
 * @return [string]
 */
function vio_register_post_type_args( $args, $post_type ) {

	if ( 'portfolio' === $post_type ) {
		$args['rewrite']['slug'] = vio_global_var( 'change_slug_portfolio', '', false );
	}

	if ( 'team' === $post_type ) {
		$args['rewrite']['slug'] = vio_global_var( 'change_slug_team', '', false );
	}

	if ( 'case-studies' === $post_type ) {
		$args['rewrite']['slug'] = vio_global_var( 'change_slug_casestudies', '', false );
	}

	return $args;
}
add_filter( 'register_post_type_args', 'vio_register_post_type_args', 10, 2 );

/**
 * Add new mimes for custom font upload
 */
if ( ! function_exists( 'vio_upload_mimes' ) ) {
	/**
	 * [vio_upload_mimes description]
	 *
	 * @param array $existing_mimes description.
	 */
	function vio_upload_mimes( $existing_mimes = array() ) {
...
add_filter( 'upload_mimes', 'vio_upload_mimes' );
...
add_action( 'admin_enqueue_scripts', 'vio_enqueue_scripts' );
 ...
 * @return void
 */
function vio_after_switch_theme() {
	if ( current_user_can( 'manage_options' ) ) {
		wp_safe_redirect( admin_url( 'themes.php?page=radiantthemes-dashboard' ) );
	}
}
add_action( 'after_switch_theme', 'vio_after_switch_theme' );
 ...
 * @return array
 */
function vio_custom_fonts( $array ) {
...
// add the filter.
add_filter( 'redux/vio_theme_option/field/typography/custom_fonts', 'vio_custom_fonts', 10, 1 );
