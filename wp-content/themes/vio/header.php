...
<?php
	$data_nicescroll_cursorcolor = ( ! empty( vio_global_var( 'scrollbar_color', 'color', true ) ) ) ? vio_global_var( 'scrollbar_color', 'color', true ) : '';
	$data_nicescroll_cursorwidth = ( ! empty( vio_global_var( 'scrollbar_width', 'width', true ) ) ) ? vio_global_var( 'scrollbar_width', 'width', true ) : '';
?>


<?php if ( ( 'default' != get_post_meta( get_the_id(), 'selectheader', true ) ) && ( get_post_meta( get_the_id(), 'selectheader', true ) ) ) { ?>
	<body <?php body_class(); ?> data-page-transition="<?php echo esc_attr( vio_global_var( 'page_transition_switch', '', false ) ); ?>" data-header-style="header-style-<?php echo esc_attr( get_post_meta( get_the_id(), 'selectheader', true ) ); ?>" data-nicescroll-cursorcolor="<?php echo esc_attr( $data_nicescroll_cursorcolor ); ?>" data-nicescroll-cursorwidth="<?php echo esc_attr( $data_nicescroll_cursorwidth ); ?>">
<?php } elseif ( ( vio_global_var( 'header-style', '', false ) ) ) { ?>
	<body <?php body_class(); ?> data-page-transition="<?php echo esc_attr( vio_global_var( 'page_transition_switch', '', false ) ); ?>" data-header-style="<?php echo esc_attr( vio_global_var( 'header-style', '', false ) ); ?>" data-nicescroll-cursorcolor="<?php echo esc_attr( $data_nicescroll_cursorcolor ); ?>" data-nicescroll-cursorwidth="<?php echo esc_attr( $data_nicescroll_cursorwidth ); ?>">
<?php } else { ?>
...
if ( ! is_user_logged_in() && ! empty( vio_global_var( 'coming_soon_switch', '', false ) ) ) {
	...
	die;
} elseif ( ! is_user_logged_in() && ! empty( vio_global_var( 'coming_soon_switch', '', false ) ) && ! empty( seolounge_global_var( 'maintenance_mode_switch', '', false ) ) ) {
	include ABSPATH . 'wp-content/plugins/radiantthemes-addons/coming-soon.php';
	die;
}
?>

	<?php if ( vio_global_var( 'preloader_switch', '', false ) ) { ?>
		<!-- preloader -->
		<div class="preloader" data-preloader-timeout="<?php echo esc_attr( vio_global_var( 'preloader_timeout', '', false ) ); ?>">
			<?php
			if ( ! empty( vio_global_var( 'preloader_style', '', false ) ) ) {
				get_template_part( 'inc/preloader/' . vio_global_var( 'preloader_style', '', false ), 'none' );
			}
			?>
		</div>
		<!-- preloader -->
	...
	<?php if ( vio_global_var( 'page_transition_switch', '', false ) ) { ?>
	...
	<!-- scrollup -->
	<?php if ( vio_global_var( 'scroll_to_top_switch', '', false ) ) { ?>
		<?php if ( ! empty( vio_global_var( 'scroll_to_top_direction', '', false ) ) ) : ?>
			<div class="scrollup <?php echo esc_attr( vio_global_var( 'scroll_to_top_direction', '', false ) ); ?>">
		<?php else : ?>
	...
	<?php if ( vio_global_var( 'gdpr_notice_switch', '', false ) ) { ?>
		<!-- gdpr-notice -->
		<div class="gdpr-notice alert alert-dismissible i-am-hidden">
			...
			<div class="row">
				<div class="col-lg-9 col-md-9 col-sm-8 col-xs-12 text-left">
					<p><?php echo esc_html( vio_global_var( 'gdpr_notice_content', '', false ) ); ?></p>
				</div>
				<div class="col-lg-3 col-md-3 col-sm-4 col-xs-12 text-center">
					<a class="btn" href="<?php echo esc_url( vio_global_var( 'gdpr_notice_button_link', '', false ) ); ?>"><?php echo esc_html( vio_global_var( 'gdpr_notice_button_text', '', false ) ); ?></a>
				</div>
			</div>
			<!-- row -->
		</div>
	...
	<?php if ( class_exists( 'ReduxFrameworkPlugin' ) ) { ?>
		<?php if ( 'full-width' === vio_global_var( 'layout_type', '', false ) ) { ?>
			<div class="radiantthemes-website-layout full-width">
		<?php } elseif ( 'boxed' === vio_global_var( 'layout_type', '', false ) ) { ?>
	...
	<?php } ?>
		...
		if ( ! class_exists( 'ReduxFrameworkPlugin' ) ) {
			...
			if ( is_404() || is_search() ) {
				if ( ( vio_global_var( 'header-style', '', false ) ) ) {
					get_template_part( 'inc/header/rt', vio_global_var( 'header-style', '', false ) );
				} else {
					get_template_part( 'inc/header/rt', 'header-style-default' );
				}
			} else {
				...
				} elseif ( ( vio_global_var( 'header-style', '', false ) ) ) {
					get_template_part( 'inc/header/rt', vio_global_var( 'header-style', '', false ) );
				} else {
					get_template_part( 'inc/header/rt', 'header-style-default' );
				}
		...
		<!-- hamburger-menu-holder -->
			...
			<?php
			if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
				if ( ( 'header-style-default' != vio_global_var( 'header-style', '', false ) ) ) {
					if (
