...
if ( ! empty( $shortcode['calltoaction_title'] ) ) {
	$output .= '<h3 style="' . $title_style . ' ' . $cta_title_font_inline_style . '">' . esc_attr( $shortcode['calltoaction_title'] ) . '</h3>';
}
if ( ! empty( $shortcode['calltoaction_content'] ) ) {
	$output .= '<p style="' . $content_style . ' ' . $cta_content_font_inline_style . '">' . wp_kses_post( $shortcode['calltoaction_content'] ) . '</p>';
}
$output .= '</div>';
$output .= '</div>';
$output .= '</div>';
...
if ( ! empty( $shortcode['calltoaction_subtitle'] ) ) {
	$output .= '<h4 style="' . $subtitle_style . ' ' . $cta_subtitle_font_inline_style . '">' . wp_kses_post( $shortcode['calltoaction_subtitle'] ) . '</h4>';
}
if ( ! empty( $shortcode['calltoaction_button_title'] ) ) {
	$button_url = ( ! empty( $cta_button_link['url'] ) ) ? $cta_button_link['url'] : '#';

	$output .= '<a style="' . $cta_button_title_font_inline_style . '" class="btn ' . esc_attr( $call_to_action_button_design ) . '" href="' . esc_url( $button_url ) . '" title="' . esc_attr( $cta_button_link['title'] ) . '" rel="' . esc_attr( $cta_button_link['rel'] ) . '">';
