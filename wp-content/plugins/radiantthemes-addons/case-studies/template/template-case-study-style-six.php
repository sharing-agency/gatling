...
$output  = '<div style="' . $case_studies_filter_font_inline_style . '" class="rt-case-study-box-filter element-six filter-style-' . esc_attr( $shortcode['case_study_filter_style'] ) . ' text-' . esc_attr( $shortcode['case_study_filter_alignment'] ) . ' ' . esc_attr( $hidden_filter ) . '">';
	...
	while ( $my_query->have_posts() ) :
		...
		if ( true == $shortcode['case_study_enable_title'] ) {
			if ( 'yes' === $shortcode['case_study_enable_zoom'] ) {
				$output .= '<h4 style="' . $case_studies_title_font_inline_style . '"><a class="fancybox" href="' . get_the_permalink() . '">' . get_the_title() . '</a></h4>';
			} else {
		...
		if ( true == $shortcode['case_study_enable_excerpt'] ) {
			$output .= '<p style="' . $case_studies_excerpt_font_inline_style . '">' . wp_trim_words( get_the_excerpt(), 30, '...' ) . '</p>';
		}
		$output .= '<a style="' . $case_studies_button_font_inline_style . '" class="arrow-link" href="' . get_the_permalink() . '">' . esc_html( $shortcode['case_study_link_button_text'] ) . '<i class="fa fa-arrow-circle-o-right"></i></a>';
		$output .= '</div>';
		$output .= '</div>';
		$output .= '</div>';
	endwhile;
