...
$output  = '<div style="' . $case_studies_filter_font_inline_style . '" class="rt-case-study-box-filter element-one filter-style-' . esc_attr( $shortcode['case_study_filter_style'] ) . ' text-' . esc_attr( $shortcode['case_study_filter_alignment'] ) . ' ' . esc_attr( $hidden_filter ) . '">';
...
if ( $my_query->have_posts() ) {
	while ( $my_query->have_posts() ) :
		...
		if ( true == $shortcode['case_study_enable_title'] ) {
			$output .= '<h4 style="' . $case_studies_title_font_inline_style . '">' . get_the_title() . '</h4>';
		}
