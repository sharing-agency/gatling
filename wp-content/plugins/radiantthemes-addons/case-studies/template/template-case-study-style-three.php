...
$output  = '<div style="' . $case_studies_filter_font_inline_style . '" class="rt-case-study-box-filter element-three filter-style-' . esc_attr( $shortcode['case_study_filter_style'] ) . ' text-' . esc_attr( $shortcode['case_study_filter_alignment'] ) . ' ' . esc_attr( $hidden_filter ) . '">';
...
if ( $my_query->have_posts() ) {
	while ( $my_query->have_posts() ) :
		...
		$output .= '<p class="category" style="' . $case_studies_category_font_inline_style . '">';
		if ( ! empty( $terms ) ) {
			foreach ( $terms as $term ) {
				if ( $term !== end( $terms ) ) {
		...
		if ( true == $shortcode['case_study_enable_title'] ) {
			$output .= '<h4 style="' . $case_studies_title_font_inline_style . '"><a href="' . get_the_permalink() . '">' . get_the_title() . '</a></h4>';
		}
		if ( true == $shortcode['case_study_enable_excerpt'] ) {
			$output .= '<p style="' . $case_studies_excerpt_font_inline_style . '">' . wp_trim_words( get_the_excerpt(), 10, '...' ) . '</p>';
		}
		$output .= '</div>';
		$output .= '<div class="pic">';
		$output .= '<img src="' . plugins_url( 'radiantthemes-addons/assets/images/Blank-Image-100x43.png' ) . '" alt="blank image" width="100" height="43">';
		...
		if ( true == $shortcode['case_study_enable_link_button'] && ! empty( $shortcode['case_study_link_button_text'] ) ) {
			...
			$output .= '<a style="' . $case_studies_button_font_inline_style . '" class="btn" href="' . get_the_permalink() . '">' . esc_html( $shortcode['case_study_link_button_text'] ) . '<i class="fa fa-angle-right"></i></a>';
			$output .= '<a style="' . $case_studies_button_font_inline_style . '" class="btn alt" href="' . get_the_permalink() . '">' . esc_html( $shortcode['case_study_link_button_text'] ) . '<i class="fa fa-angle-right"></i></a>';
			$output .= '</div>';
		}
		$output .= '</div>';
		$output .= '</div>';
