...
$output  = '<div style="' . $case_studies_filter_font_inline_style . '" class="rt-case-study-box-filter element-seven filter-style-' . esc_attr( $shortcode['case_study_filter_style'] ) . ' text-' . esc_attr( $shortcode['case_study_filter_alignment'] ) . ' ' . esc_attr( $hidden_filter ) . '">';
...
if ( $my_query->have_posts() ) {
	while ( $my_query->have_posts() ) :
		...
		$output .= '<img src="' . plugins_url( 'radiantthemes-addons/assets/images/Blank-Image-100x65.png' ) . '" alt="blank image" width="100" height="65">';
		...
		if ( true == $shortcode['case_study_enable_excerpt'] ) {
			$output .= '<p style="' . $case_studies_excerpt_font_inline_style . '">' . wp_trim_words( get_the_excerpt(), 20, '...' ) . '</p>';
		}
		$output .= '</div>';
		$output .= '</div>';
		$output .= '</div>';
