...
if ( class_exists( 'WPBakeryShortCode' ) && ! class_exists( 'Radiantthemes_Style_Clients' ) ) {
	...
	class Radiantthemes_Style_Clients extends WPBakeryShortCode {
					...
					'params'      => array(
						array(
							...
							'value'      => array(
								...
								esc_html__( 'Style Five (Boxed Background Bordered)', 'radiantthemes-addons' )              => 'five',
							),
							'std'        => 'one',
							'weight'     => 2,
						),
		...
		public function radiantthemes_clients_style_func( $atts, $content = null, $tag ) {
			...
			if ( 'all' === $shortcode['client_category_list'] ) {
				$client_category = '';
			} else {
				$client_category = array(
					array(
						...
						'field'    => 'slug',
						'terms'    => esc_attr( $shortcode['client_category_list'] ),
					),
				);
			}
